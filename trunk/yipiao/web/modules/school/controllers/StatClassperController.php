...
class StatClassperController extends CommonController
{
		        ...
		        foreach ($uidarr as $uid)
		        {
		        	...
		        	if (!empty($record))
					{
						...
						foreach ($rows as $k => $v ){
							...
							$scorejson["s".strval($classstatInfo["subjectid"])."-c"] = $classstatInfo["avg"].'-s'.strval($classstatInfo["subjectid"]);
							//查询个人成绩
							$recordScoreList = InfoExamScore::model()->findAll("examid = :ExamID and SubjectID = ".$classstatInfo["subjectid"]." and uid = ".$studentInfo["uid"],
			        		array('ExamID'=>$examid));
			        		foreach ($recordScoreList as $recordScore)
			        		{
			        			$scoreInfo = array_change_key_case((array)$recordScore->getAttributes(), CASE_LOWER);
				         		$scorejson["s".strval($scoreInfo["subjectid"])] = $scoreInfo["score"].'-s'.strval($classstatInfo["subjectid"]);
			        		}
			        		//查询年级成绩
			        		$connection=Yii::app()->db; 
							$sql1="select * from info_exam_grade_stat where ExamID = ".$examid." and GradeID =".$grade_id." and SubjectID = ".$classstatInfo["subjectid"];
							$rows1=$connection->createCommand ($sql1)->query();
							foreach ($rows1 as $k1 => $v1 ){
								$gradestatInfo = array_change_key_case($v1, CASE_LOWER);
								$scorejson["s".strval($classstatInfo["subjectid"])."-g"] = $gradestatInfo["avg"];
							}
						}
			            $result['data'][] = array_change_key_case($scorejson, CASE_LOWER);
					}
